//*************************************************************//
// SINGLE PHASE UNITY POWER FACTOR CONVERTER BASED ON SINE PWM //
//             AND UNBALANCED D-Q CONTROL                      //
//*************************************************************//
	...
	if(pll_Flag) //12Khz _PLL_count
		{
          ...
          //Filter fictious power output value
          FOF_PreOut = p_FOFout;
          ...
          if(P_PIout <= OSC_Fmin) P_PIout = OSC_Fmin;
              ...
              if(VDCPI_out >= current_max) VDCPI_out = current_max;             //limit inverter current ref.
              if(VDCPI_out <= current_min) VDCPI_out = current_min;    
              //

              I_ref = VDCPI_out;  //set ref. current
         		    ...
         		    if(theta >= theta_2PI)        //reset harmonic oscillator
            		    {
           				...
           				theta = 0;
              					...
              					else asmHARMONIC(); //sine cosine are interchanged for power PLL
